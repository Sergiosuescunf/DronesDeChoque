...
if os_name == 'posix':
    FILE_NAME = 'EdificioEntreno.x86_64'
    CLEAR_COMMAND = 'clear'
elif os_name == 'nt':
...
def CalculateExploredZones(id):
    return w_zones_score * len(Models[id].explored_zones)
...
def Train():
    ...
    channel.set_configuration_parameters(width=1280, height=960)
...
def ShowPopulation():
    ...
    env = UnityEnvironment(file_name="EdificioEntrenoTest.x86_64", seed=1, side_channels=[channel])
    env.reset()
